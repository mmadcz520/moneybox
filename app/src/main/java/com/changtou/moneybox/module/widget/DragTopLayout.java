...
public class DragTopLayout extends FrameLayout {
    ...
    private void bindId(View view) {
        topView = (ScrollView)view.findViewById(topViewId);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        topView.layout(left, getHeight() + contentTopTemp, right,
                getHeight() + contentTopTemp + topViewHeight);
...
//                return Math.min(top, getPaddingTop() + collapseOffset);
//            } else {
//                return Math.min(topViewHeight, Math.max(top, getPaddingTop() + collapseOffset));
//            }
            return Math.min(0,Math.max(top, (-getHeight())));
        }

        /**
         * page 1 移动到位时载入 page2 判断
         *
         *
         * @param releasedChild
         * @param xvel
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (yvel < 0)
            {
                if(Math.abs(contentTop) >(contentHeight/8))
                {
                    top = (-getHeight());
                }
                else
            ...
            {
                if(Math.abs(contentHeight + contentTop)>(contentHeight/8))
                {
                    top = 0;
                }
                else
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        try {
            ...
            if((ratio == -1.0))
            {
                intercept = (topView.getScrollY() == 0);
            }
            else
            {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //开启或闭合时焦点转移
        if (action == MotionEvent.ACTION_MOVE && ratio == -1.0f) {
            ...
            topView.dispatchTouchEvent(event);
        }
